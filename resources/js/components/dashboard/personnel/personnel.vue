<template>
    <div class="w-full flex justify-center">
        <div class="w-3/4 flex justify-center border bg-white drop-shadow p-2">
            <div class="w-full">
                    ...
                    <div>
                        <div class="w-full">
                            <a-button class="bg-gray-200 mb-2 w-full" size="small" @click="openLA = true">Leave
                                Credits</a-button>
                            <a-modal v-model:open="openLA" title="Application for Leave" width="500px"
                                style="top: 100px" :maskClosable="false" @ok="handleOkLA('formValidateLA')">
                                <template #footer>
                                    <a-button key="submit" class="bg-gray-200 mb-2"
                                        @click="handleOkLA('formValidateLA')" :loading="loading">Submit</a-button>
                                </template>
                                <Form ref="formValidateLA" :model="formValidateLA" :rules="ruleValidateLA"
                                    ...
                                    <FormItem label="No. of Days" prop="noOfDays">
                                        <Input v-model="formValidateLA.noOfDays"
                                            placeholder="Enter No. of Days"></Input>
                                    </FormItem>
...
export default defineComponent({
    ...
    methods: {
        ...
        async handleOkLA(name) {
            ...
            thiss.formValidateLA.id = thiss.data.id
        ...
        async handleOkCP(name) {
            ...
            thiss.formValidateCP.id = thiss.data.id
            this.$refs[name].validate((valid) => {
                if (valid) {
    ...
    async mounted() {
        ...
        try {
            const response = await axios.get(`/api/admin/getUserPersonnel`, { headers });
